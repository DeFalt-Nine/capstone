...
const TouristSpotsPage: React.FC = () => {
  ...
  const handleCategoryChange = (cat: string) => {
      ...
      trackEvent('filter', `category_${cat}`, viewMode === 'tourist' ? '/tourist-spots' : '/dining-spots');
  };
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {filteredItems.map((item, index) => {
          ...
          <AnimatedElement key={item._id || index} delay={index * 100}>
            <div 
              className="bg-white rounded-2xl shadow-lg hover:shadow-2xl hover:shadow-lt-blue/10 overflow-hidden group cursor-pointer flex flex-col h-full transform transition-all duration-300 hover:-translate-y-2 border border-slate-100 relative"
  ...
  return (
    <>
      <section id="gallery" className="min-h-screen bg-slate-50 py-20 md:py-32 overflow-hidden">
          ...
          <AnimatedElement delay={100}>
              <div className="max-w-4xl mx-auto mb-16 space-y-6">
                  ...
                  <div className="relative group z-20">
                      <div className={`absolute -inset-1 bg-gradient-to-r ${viewMode === 'tourist' ? 'from-lt-blue to-teal-400' : 'from-lt-orange to-red-400'} rounded-full blur opacity-20 group-hover:opacity-40 transition duration-1000 group-hover:duration-200`}></div>
                      
                      <div className="relative bg-white rounded-full shadow-lg flex items-center p-2 border border-slate-100">
                          <div className={`w-12 h-12 rounded-full flex items-center justify-center text-xl ml-1 ${viewMode === 'tourist' ? 'bg-lt-blue/10 text-lt-blue' : 'bg-lt-orange/10 text-lt-orange'} transition-colors`}>
                              <i className="fas fa-search"></i>
                          </div>
                          <input 
                            type="text" 
                            placeholder={viewMode === 'tourist' ? "Search for a destination..." : "Search for restaurants..."}
                            className="w-full px-4 py-3 text-slate-700 font-medium focus:outline-none bg-transparent placeholder-slate-400 text-lg"
                            value={searchQuery}
                      ...
                      <div className="bg-slate-200 p-1.5 rounded-xl flex shrink-0 shadow-inner">
                          <button 
                            ...
                            className={`px-6 py-2.5 rounded-lg text-sm font-bold transition-all duration-300 flex items-center gap-2 ${
                                viewMode === 'tourist' 
                                ? 'bg-white text-lt-blue shadow-sm scale-100' 
                          ...
                          <button 
                            onClick={() => handleModeToggle('dining')}
                            className={`px-6 py-2.5 rounded-lg text-sm font-bold transition-all duration-300 flex items-center gap-2 ${
                                viewMode === 'dining' 
                                ? 'bg-white text-lt-orange shadow-sm scale-100' 
                                : 'text-slate-500 hover:text-slate-700 hover:bg-slate-200/50'
                            }`}
                          >
                          ...
                          <div className="flex gap-2 justify-center md:justify-end">
                              {categories.map(cat => (
                                  <button
                                    ...
                                    className={`whitespace-nowrap px-5 py-2.5 rounded-full text-xs font-bold transition-all border transform active:scale-95 ${
                                        activeCategory === cat 
                                        ? `${viewMode === 'tourist' ? 'bg-lt-blue border-lt-blue' : 'bg-lt-orange border-lt-orange'} text-white shadow-md` 
      ...
      {selectedSpot && (
        <TouristSpotModal 
          spot={selectedSpot} 
          spotType={viewMode} // Pass the view mode directly
